...
def train_model(train_texts, train_labels, model, tokenizer, training_args, model_name, ticker, year_range):
    ...
    # Reload the model
    model = AutoModelForSequenceClassification.from_pretrained(finbert_model_name, num_labels=3, ignore_mismatched_sizes=True).to(device)
    return model
...
for ticker in tickers:
    ...
    finbert_model_path = f"{ticker_dir}\\finbert_model_{ticker}_{min(train_years)}-{max(train_years)}.bin"
    lr_model_path = f"{ticker_dir}\\lr_model_{ticker}_{min(train_years)}-{max(train_years)}.joblib"
    model.save_pretrained(finbert_model_path)
