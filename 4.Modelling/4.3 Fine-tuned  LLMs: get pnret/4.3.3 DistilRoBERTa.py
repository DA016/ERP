...
distilroberta_model_name = "mrm8488/distilroberta-finetuned-financial-news-sentiment-analysis"
...
def train_model(train_texts, train_labels, model, tokenizer, training_args, model_name, ticker, year_range):
    ...
    # Reload the model
    model = AutoModelForSequenceClassification.from_pretrained(distilroberta_model_name, num_labels=3, ignore_mismatched_sizes=True).to(device)
    return model

# Function to get sentiment scores
def get_sentiment_scores(headlines, model, tokenizer, device):
    model.to(device)
    sentiments = []
    for headline in headlines:
        ...
        # Assign a sentiment score based on probabilities
        score = probabilities[2] - probabilities[0]  # Positive probability minus negative probability
...
for ticker in tickers:
    ...
    for train_years, test_years in train_test_splits:
        ...
        ticker_data.loc[:, 'fscore'] = get_sentiment_scores(all_years_texts, model, tokenizer, device)
        ...
        # Save test results
        test_output_path = f"{base_path}{model_path}\\test\\{ticker}_Ticker-Year-Week-wheadline-ndirection-nret-fscore-pnret-pndirection-{min(train_years)}-{max(train_years)}.csv"
        test_data.to_csv(test_output_path, index=False)
    
    # Save all results for the current ticker
    ticker_dir = f"{base_path}{model_path}\\test\\{ticker}"
    os.makedirs(ticker_dir, exist_ok=True)
    distilroberta_model_path = f"{ticker_dir}\\distilroberta_model_{ticker}_{min(train_years)}-{max(train_years)}.bin"
    lr_model_path = f"{ticker_dir}\\lr_model_{ticker}_{min(train_years)}-{max(train_years)}.joblib"
    model.save_pretrained(distilroberta_model_path)
